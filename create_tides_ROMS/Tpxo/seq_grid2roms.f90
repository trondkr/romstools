subroutine seq_grid2roms(unit,identi, gparam, imax,jmax, lon_seq, lat_seq, angle,topo)

  integer, intent(in), dimension(20) :: identi
  real, intent(in), dimension(6)     :: gparam
  integer, intent(in)                :: imax, jmax, unit
  real, intent(inout),dimension(0:imax+1,0:jmax+1) :: lon_seq, lat_seq, angle
  real, intent(inout),dimension(imax,jmax) :: topo
...
!     print *, 'Input grid is not recognized'
     goto 300
